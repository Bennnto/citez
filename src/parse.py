...
def p_type(p):
    """type : I8
            ...
            | MUL type"""
    if len(p) == 3:
        p[0] = Pointer_Type_Node(base_type=p[2])
    else :
...
def p_if_stmt(p):
    ...
    | IF expression LBRACE statements RBRACE ELSE if_stmt"""
    if len(p) == 6:
        p[0] = If_Node(cond=p[2], if_block=p[4], else_block=None)
    elif len(p) == 8:
        p[0] = If_Node(cond=p[2], if_block=p[4], else_block=[p[7]])
    else:
...
def p_for_stmt(p):
    ...
    if len(p) == 10:
        p[0] = For_Node(
            init=p[2], cond=p[4], increment=p[6], body=p[8], is_classic=True
        )
    elif len(p) == 8:
...
def p_argument(p):
    ...
    if len(p) == 2:
        p[0] = Argument_Node(name="", value=p[1])
    else:
...
def p_expression_pointer(p):
    """expression : MUL type expression"""
    p[0] = Pointer_Type_Node(base_type=p[2])


def p_expression_address(p):
    """expression : ADDRESS IDENT"""
    p[0] = Address_Node(p[2])
